...
basedir='/Users/claudiaba/Documents/ColHA_Porosity/ColHAX_Image_Segmentation/250X/';
%Load file input location
...
for i=1:G
    ...
    for j=1:T
        ...
        savename= fullfile(savedir,'/segmentimage/','/250Xsegmented/',geltype{i} + "_" + trialsdir(j,1).name(1:end-4)+'.fig');% end-4 removes .tif
        img1=imread([trialsdir(j,1).folder,filesep,trialsdir(j,1).name]);
        img1crop=img1(1:ycrop,:); 

        if i <= 2 %Collagens only
            %Smaller mask
        ...
        elseif i <= 4
            ...
            %SAVE TO EXCEL FILE 
            writematrix(poresize(:,:),'poresize250X.xlsx','Sheet',geltype{i}, 'Range','A2:E2000')
